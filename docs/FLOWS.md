...
*Companion to [SPEC.md](./SPEC.md) and [DESIGN.md](./DESIGN.md). The two tasks that define the product from the user's chair.*
...
5. Board boots, connects to broker, announces:
   device_id (from eFuse MAC) + platform_type + capabilities + fw version
6. Token valid → AUTO-ENROLL: device is trusted & active immediately
7. Dashboard: user names it, assigns a group/tag
→ Managed, online device
```

**Decisions:**
- **Detection/flashing = in-dashboard Web Serial** (`esptool-js`, the ESP Web Tools stack). Plug in → one click to grant the port → chip auto-detected → flash. Zero install, self-host-friendly.
  - *Caveats:* Chrome/Edge only; needs `localhost`/HTTPS; one mandatory click (browsers forbid silent port enumeration); no batch enrollment in v1 — **CLI flasher for batch/CI is post-v1.**
...
- **Trust = auto-enroll via token.** Zero-friction, batch-friendly.
  - *Mitigations for leaked-token risk:* tokens are **short-lived, group-scoped, and revocable**; every enrolled device is visible and removable in the dashboard; per-device certs (mTLS) are the post-v1 hardening step.
...
3. SIMULATE (advisory): boot artifact + run self-test → pass / warn+override
...
**Decisions:**
- **Artifact source = user's own toolchain.** They build the `.bin` (idf.py / PlatformIO / Arduino); the server never builds (develop pillar is out of scope). Artifacts are opaque + versioned.
...
VCS integration is an **automated artifact producer** feeding the same pipeline; steps 2–7 above are unchanged.
...
- **Identity** announced in Flow 1 step 3 is the `platform_type` + capabilities the server matches against in Flow 2 step 2.
