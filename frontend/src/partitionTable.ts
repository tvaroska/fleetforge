...
//
// Exists for one reason: the flasher needs to wipe `nvs` without touching `phy_init`, and
// the only honest source for where those two live is the table being written to the board
// in the same operation. Hardcoding 0x9000 would be right for `ab-4m-v1` today and silently
// wrong for the next layout — and "silently wrong" here means erasing the wrong 24 KB.
//
